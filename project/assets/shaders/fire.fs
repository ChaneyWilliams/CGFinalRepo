...
void main()
{
    ...
    float stepTime = 0.5;
    float block = floor(TIME / stepTime);
    float frame = mod(block * frameJumper, float(frameCount));

    float frameHeight = 1.0 / float(frameCount);

    vec2 uv = fragmentUV;
    uv.y = 1.0 - uv.y;
    uv.y = uv.y * frameHeight + frame * frameHeight;

    vec4 albedoTex = useAlbedoMap ? texture(albedoMap, uv) : vec4(1.0);
    if (albedoTex.a < 0.1)
        discard;

    vec4 albedo = albedoValue * albedoTex;

    float pulse = 0.6 + firePulse * 0.4;
    albedo.rgb *= pulse;
    ...
    litColor += albedoLinear * firePulse * 1.5;
    ...
    for (int i = 0; i < pointLightCount && i < MAX_POINT_LIGHTS; ++i)
    {
        ...
        vec3 lightDir = (distanceToLight > 0.0001)
        ...
        litColor += EvaluateLightContribution(
            n,
            viewDir,
            lightDir,
            lightColor,
